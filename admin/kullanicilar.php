<?php
/**
 * Admin Paneli - Kullanıcı Yönetimi (Şoförler)
 */
...
$pageTitle = 'Şoför Yönetimi - Admin Paneli';
    ...
    <div class="table-responsive">
        <table class="table">
            ...
            <tbody>
                <?php if (empty($users)): ?>
                    <tr>
                        <td colspan="7" class="text-center p-5 text-gray">Şoför bulunamadı.</td>
                    </tr>
                <?php else: ?>
...
<script>
    ...
    async function sendResetWhatsApp(phone, name) {
        if (!confirm(`${name} kullanıcısına WhatsApp üzerinden şifre sıfırlama linki gönderilsin mi?`)) return;

        try {
            const response = await fetch('api/generate-reset-link.php', {
                method: 'POST',
                headers: {
